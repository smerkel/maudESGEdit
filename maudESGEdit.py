...
#
# Beam center is not corrected in this software because esg file generated by MAUD already include a first beam center correction
# based on user input when creating the esg file. This is sufficient for our needs
#
#################################################################
...
class inclinedDetectorDialog(PyQt5.QtWidgets.QDialog):
	def __init__(self, parent, detDistance, detTTheta=0.0, detTilt=0.0, detRotation=0.0, detEta=0.0):
		"""
		Send detector distance (in mm) and detector angles in degrees (2theta, tilt, rotation, and eta as they are called in the MAUD gui)
		"""
		...
		label.setText("Provide info on detector geometry... 1 degree precision sufficient.")
	...
	def accept(self):
		# Before we accept to close the window, we make sure all that should be numbers actually are numbers
